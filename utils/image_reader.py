...
def read_images_from_disk_infer(input_queue, input_size, random_scale,
    ...
    if input_size is not None:
        ...
        # Randomly scale the images and labels.
        if random_scale:
            img = image_scaling_only(img)

        # Randomly mirror the images and labels.
        if random_mirror:
            img = image_mirroring_only(img)
...
class ImageReaderInfer(object):
    '''Generic ImageReader which reads images and corresponding segmentation
       masks from the disk, and enqueues them into a TensorFlow queue.

       Image reader for interence (test)
    '''
    ...
    def __init__(self, data_dir, data_list, data_id_list, input_size, random_scale,
        ...
        self.image = read_images_from_disk_infer(self.queue, self.input_size, random_scale,
                                                                  random_mirror)
